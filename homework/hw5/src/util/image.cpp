...
Buffer2D<float> ReadFloatImageLayer(const std::string& filename, const std::string& layername) {
  int width, height;
...
Buffer2D<Float3> ReadFloat3ImageLayer(const std::string& filename, const std::string& layername) {
  int width, height;
...
void WriteFloatImage(const Buffer2D<float>& imageBuffer, const std::string& filename) {
  WriteImage(filename, imageBuffer.m_width, imageBuffer.m_height, 1, (float*)imageBuffer.m_buffer.get());
}

void WriteFloat3Image(const Buffer2D<Float3>& imageBuffer, const std::string& filename) {
  WriteImage(filename, imageBuffer.m_width, imageBuffer.m_height, 3, (float*)imageBuffer.m_buffer.get());
}
