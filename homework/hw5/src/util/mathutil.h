...
inline float SafeSqrt(const float& v) {
  return std::sqrt(std::max(v, 0.f));
}
...
inline Float3 Sqr(const Float3& a) {
  return Float3(Sqr(a.x), Sqr(a.y), Sqr(a.z));
}
...
inline float SqrLength(const Float3& a) {
...
inline float SqrDistance(const Float3& a, const Float3& b) {
